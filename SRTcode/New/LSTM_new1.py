...
for X in range(8):
    for Y in range(8):
        ...
        Conv1[X,Y]=score[1] * (mmn._max - mmn._min) / 2. * m_factor
        ...
        Xptest=XXp_test[:,:,X,Y].reshape([-1,1,FF2])
        XTEST=Xptest[:-9]
        for i in range(8):
            XTEST=np.concatenate((XTEST,Xptest[i+1:-(8-i)]),axis=1)
        XTEST=np.concatenate((XTEST,Xptest[9:]),axis=1)
        
        YTRAIN=Y_train[9:,:,X,Y].reshape([-1,2])
        YTEST=Y_test[:,:,X,Y].reshape([-1,2])
        ...
        LSTM1[X,Y]=score[1] * (mmn._max - mmn._min) / 2. * m_factor
        
        print(X,Y)
        print('Conv1',Conv1[X,Y])
        print('LSTM1',LSTM1[X,Y])
        np.save('Conv1.npy',Conv1)
        np.save('LSTM1.npy',LSTM1)
